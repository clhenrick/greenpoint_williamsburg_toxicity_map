...
app.map = (function(w, d, $, _) {
    ...
    function initStaticlayers() {
        ...
        var queries = {
            queries: [{
                id: 'nycd_url',
                query: "https://nag-brooklyn.cartodb.com/api/v2/sql?format=geojson&q=SELECT the_geom FROM nycd_2015 where borocd = 301&api_key=e0ff9a25a3dbac01f2bb2eb8b30e8af922282856"
            }, {
                id: 'local_truck',
                query: "https://nag-brooklyn.cartodb.com/api/v2/sql?format=geojson&q=SELECT the_geom FROM local_truck_route_cd1&api_key=e0ff9a25a3dbac01f2bb2eb8b30e8af922282856"
            },/*{
                id: 'ttruck_url',
                query: "https://nag-brooklyn.cartodb.com/api/v2/sql?format=geojson&q=SELECT the_geom FROM through_truck_routes_nyc_september_2009 ORDER BY the_geom <-> ST_SetSRID(ST_MakePoint( -73.945812,40.717283),4326) ASC LIMIT 4000&api_key=e0ff9a25a3dbac01f2bb2eb8b30e8af922282856"
            },*/
             {
                id: 'zip_url',
                query: "https://nag-brooklyn.cartodb.com/api/v2/sql?format=geojson&q=SELECT the_geom FROM nyc_zip_code_tabulation_areas_polygons ORDER BY the_geom <-> ST_SetSRID(ST_MakePoint( -73.945812,40.717283),4326) ASC LIMIT 30&api_key=e0ff9a25a3dbac01f2bb2eb8b30e8af922282856"
            }]
        };
        for (var i=0; i < 4; i++) {
